// في ملف create_new_event_view.dart
...
class EditEvent extends StatefulWidget {
  final EventDataModel?
      eventDataModel; // ✨ New: Make the eventDataModel optional

  const EditEvent(
      {super.key, this.eventDataModel}); // ✨ New: Update the constructor
...
class _EditEventState extends State<EditEvent> {
  ...
  // ✨ New: Find the correct category index based on the event data
  int _findCategoryIndex(String categoryName) {
    return eventCategories
        .indexWhere((cat) => cat.eventCategoryName == categoryName);
  }

  @override
  void initState() {
    super.initState();
    if (widget.eventDataModel != null) {
      // If we are in edit mode, pre-fill the fields with existing event data
      _titleController.text = widget.eventDataModel!.eventTitle!;
      _descriptionController.text = widget.eventDataModel!.eventDescription!;
      selectedDate = widget.eventDataModel!.eventDate!;
      // Set the selected tab based on the event's category
      int categoryIndex =
          _findCategoryIndex(widget.eventDataModel!.eventCategory!);
      if (categoryIndex != -1) {
        selectedTap = categoryIndex;
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.eventDataModel != null
            ? "Edit event"
            : "Create new event"), // ✨ New: Dynamic title
      ),
            ...
            children: [
              ...
              ElevatedButton(
                    ...
                    if (formKey.currentState!.validate()) {
                      if (selectedDate != null) {
                        EventDataModel data = EventDataModel(
                          eventID: widget.eventDataModel!.eventID,
                        ...
                        if (widget.eventDataModel != null) {
                          ...
                          await FirebaseFirestoreService.updateEvent(data);
                          EasyLoading.dismiss();
                          navigatorKey.currentState!.pop();
                          SnackBarService.showSuccessMessage(
                              "Event was successfully updated");
                        } else {
                          // Existing logic for creating a new event
                          await FirebaseFirestoreService.createNewEvent(data);
                          EasyLoading.dismiss();
                          navigatorKey.currentState!.pop();
                          SnackBarService.showSuccessMessage(
                  ...
                  child: Row(
                    ...
                    children: [
                      Text(
                        widget.eventDataModel != null
                            ? "Update Event"
                            : "Add Event", // ✨ New: Dynamic button text
                        style: theme.textTheme.titleMedium?.copyWith(
                          fontSize: 16,
                          fontWeight: FontWeight.bold,
                          color: ColorPalette.white,
